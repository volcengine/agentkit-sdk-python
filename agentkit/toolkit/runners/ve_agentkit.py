...
class VeAgentkitRuntimeRunner(Runner):
    ...
    def status(self, config: VeAgentkitRunnerConfig) -> StatusResult:
        ...
        try:
            ...
            # Get API key only for key_auth type
            if (
            ...
            if runtime.status == RUNTIME_STATUS_READY and public_endpoint:
                # Only try ping with API key if using key_auth and have a valid API key
                if (
                    ...
                    # For JWT auth, skip ping check as we don't have a token here
                    ping_status = None
    ...
    def invoke(
        ...
        try:
            ...
            # For key_auth: auto-set Authorization header if not provided
            # For custom_jwt: user must provide Authorization header with OAuth token
            if not headers.get("Authorization"):
                if is_jwt_auth:
                    error_msg = "Authorization header with OAuth token is required for JWT authentication. Please provide headers={'Authorization': 'Bearer <your_oauth_token>'}."
                    logger.error(error_msg)
                    return InvokeResult(
                        success=False, error=error_msg, error_code=ErrorCode.AUTH_FAILED
                    )
    ...
    def _build_authorizer_config_for_create(
        ...
        """Build authorizer configuration for Runtime creation based on auth type.

        Args:
            config: Runner configuration containing auth type and related settings.

        Returns:
            AuthorizerForCreateRuntime: The authorizer configuration for Runtime creation.
        """
    ...
    def _build_authorizer_config_for_update(
        ...
        """Build authorizer configuration for Runtime update based on auth type.

        Args:
            config: Runner configuration containing auth type and related settings.

        Returns:
            AuthorizerForUpdateRuntime: The authorizer configuration for Runtime update.
        """
        if config.runtime_auth_type == AUTH_TYPE_CUSTOM_JWT:
            # OAuth2/JWT authentication
    ...
    def _update_existing_runtime(self, config: VeAgentkitRunnerConfig) -> DeployResult:
        ...
        try:
            ...
            # Build authorizer configuration based on auth type
            authorizer_config = self._build_authorizer_config_for_update(config)
