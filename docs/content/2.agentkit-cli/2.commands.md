...
`agentkit list credentials` 调用 `ListInboundAuthConfigs` 列出当前凭证可见的全部凭证（inbound auth config）。命令会自动按 `PageNumber` / `PageSize` 翻页。
...
| `--region` | 区域覆盖。默认取 `VOLCENGINE_AGENTKIT_REGION` / `VOLCENGINE_REGION` / 全局配置。 | `--region cn-beijing` |
| `--output` | 输出格式：`table`（默认）、`json`、`yaml`。 | `--output json` |
| `--quiet`, `-q` | 仅打印 `InstanceId`，便于脚本消费。 | `--quiet` |
| `--no-color`, `-nc` | 关闭表格/面板的彩色输出。 | `--no-color` |
...
```bash
# 创建记忆集合
# provider-type 默认值随当前 cloud provider 生效；BytePlus 仅支持 VIKINGDB_MEMORY。
# 可通过 agentkit memory provider-types 查看当前支持的 Provider 类型。
