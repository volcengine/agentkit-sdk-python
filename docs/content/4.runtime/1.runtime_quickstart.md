...
> 我们推荐使用 `uv` 或 `python3 -m venv` 来管理您的 Python 虚拟环境。以下示例将使用 `uv`。
> 
> `uv` 安装指南请参考：[uv installation](https://docs.astral.sh/uv/getting-started/installation/)
...
这会自动生成一个包含 `simple_agent.py` 和 `requirements.txt` 的项目结构，让您能立即开始开发。
...
> **重要提示**：请务必将 `your_ak` 和 `your_sk` 替换为您的真实凭证。
...
2. **平台部署**：根据[快速开始]指南，通过`agentkit config`和`agentkit launch`命令，配置和部署应用。
