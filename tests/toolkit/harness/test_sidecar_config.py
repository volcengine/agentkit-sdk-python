...
def test_runtime_wheel_is_available_only_through_harness_sidecar_extra() -> None:
    ...
    assert not any(runtime_requirement in item for item in base_dependencies)
    assert any(runtime_requirement in item for item in sidecar_extra)
